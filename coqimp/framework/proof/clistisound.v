...
Lemma cli1_rule_sound :
  forall Spec sd  li I r ri isr is  i aop t  P,  
    GoodFrm P ->
    RuleSem Spec sd li I r ri 
            ( <||aop||> ** OS[isr, true, is, nil] ** (ATopis i) ** [| i <= INUM|] ** P) 
            (sprim cli)
...
Proof.
  ...
  exists aop OO ((x0, x1, Ml, x4, (false, si, nil)) :taskst). 
  ...
  assert (joinmem (x0, x1, x8, x4, (false, si, nil)) x6 (x0, x1, Ml, x4, (false, si, nil))).
  unfolds; join auto.
  ...
  join auto.
  simp join.
  exists ((x0, x1, x10, x4, (false, si, nil)):taskst).
  exists x.
  exists x6 x7.
  splits; eauto.
  constructors.
  ...
  simpl ;split; auto.
  unfolds; auto.
  assert (joinmem (x0, x1, x8, x4, (false, si, nil)) x5 (x0, x1, x10, x4, (false, si, nil))).
  unfolds; join auto.
...
Lemma cli2_rule_sound : 
  forall Spec sd  I r ri isr is aop li t P ,  
    RuleSem Spec sd li I r ri 
...
Proof.
  ...
  unfold inv_ieoff in Hoff.
  sep split in Hoff.
  exists aop0 OO  ((x0, x1, x8, x4, (false, si,  nil)):taskst).
  exists Ms O Os.
  splits; eauto.
...
Lemma sti1_rule_sound : 
  forall Spec sd  I r ri isr is   i aop t li P ,  
    GoodFrm P ->
    P ==> CurLINV li t->
    RuleSem Spec sd  li I r ri 
            (<|| aop ||>  **
...
Proof.
  ...
  simp join.
  exists aop0 OO. 
  exists ((x4, x5, x0, x8, (true, si, nil)):taskst).
  exists x14 x3 x10.
  splits; eauto.
  constructors.
  unfolds.
...
Lemma sti2_rule_sound :  
  forall Spec sd  I r ri isr is   aop t li P ,  
    RuleSem Spec sd  li I r ri 
            (<|| aop ||>  **
                 OS [isr, true, is, nil ]  ** P)
            (sprim sti)
            (<|| aop ||>  ** OS [isr, true, is, nil] ** P ) t.
Proof.
  introv    Hsat.
  ...
  unfold inv_ieon in Hon.
  sep split in Hon.
  exists aop0 OO  ((x0, x1, x8, x4, (true, si, nil)):taskst).
  exists Ms O Os.
  splits; eauto.
  constructors.
  unfolds.
