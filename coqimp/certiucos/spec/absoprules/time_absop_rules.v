...
Proof.
   ...
   * eauto.
   * 2:eauto.
   * instantiate (O'0 := empabst).
   * 2:eauto.
   * 
   * 2:apply eqdomO_same.
   * 2:apply eqtid_same.
   * 4:sep auto.
   * eapply disjoint_empabst.
   * apply map_join_emp.
   * apply map_join_emp. *)
Qed.
...
Lemma  OSTimeDly_high_level_step_1 :
  forall i P sch,
    i = Int.zero ->
    can_change_aop P ->
    absinfer sch (<||tmdlycode (Vint32 i :: nil)||> ** P )
             ( <|| END None ||>  ** P).
Proof.
  intros.
  subst i.
...
Lemma OSTimeDly_high_level_step_2 :
  forall i P ecbls tcbls t ct pri  msg sch,
    get tcbls ct = Some (pri, rdy, msg) ->
    Int.ltu Int.zero i = true ->
    can_change_aop P ->
    absinfer sch (<||tmdlycode (Vint32 i :: nil)||> ** HECBList ecbls ** HTCBList tcbls ** HTime t ** HCurTCB ct ** P)
             ( <|| isched;; END None ||> ** HECBList ecbls ** HTCBList (set tcbls ct (pri, wait os_stat_time i, msg)) ** HTime t ** HCurTCB ct ** P).
Proof.
  infer_solver 1%nat.
  (* intros.
   * unfold tmdlycode.
   ...
   * unfolds.
   * intros.
   * exists  (set O abtcblsid (abstcblist (set tcbls ct (pri, wait os_stat_time i, msg)))).
   * exists (END None).
   * splits.
   * hmstep_solver.
...
Lemma OSTimeDly_high_level_step_4 :
  forall i P ecbls tcbls t ct st msg sch,
    get tcbls ct = Some (Int.repr OS_IDLE_PRIO, st, msg) ->
    can_change_aop P ->
    absinfer sch (<||tmdlycode (Vint32 i :: nil)||> ** HECBList ecbls ** HTCBList tcbls ** HTime t ** HCurTCB ct ** P)
...
Lemma OSTimeDly_high_level_step_3 :
  forall i P ecbls tcbls t ct pri st msg sch,
...
(*
(* ------------------- OS Q Msg Get -------------------------*)
Lemma OSQGetMsg_high_level_step :
  forall P prio state msg hecbls htcbls htime hcurtcb sch,
...
(* ------------------- OS Q Accept -------------------------*)
Lemma qacc_absimp_null:
  forall P, can_change_aop P -> absimp sch
...
Lemma qacc_absimp_no_q:
  forall P mqls x, can_change_aop P ->  (~ exists a b wl,EcbMod.get mqls x = Some (absmsgq a b,wl)) -> absimp sch
...
Lemma OSQPend_high_step_null :
  forall qid timeout P,
    ...
    can_change_aop P ->
    absimp sch ( <|| rapiop qpendapi (qid :: Vint32 timeout :: nil) ||>  ** P)
...
Lemma OSQPend_high_step_no_q_err :
  forall qid timeout ecbls P,
    ...
    can_change_aop P ->
    absimp sch ( <|| rapiop qpendapi (Vptr qid :: Vint32 timeout :: nil) ||>  ** HECBList ecbls ** P)
...
Lemma OSQPend_high_step_get_succ :
  forall qid timeout msg ml n wl ecbls tls t tcur prio  m P,
    ...
    can_change_aop P ->
    absimp sch  ( <|| rapiop qpendapi (Vptr qid :: Vint32 timeout :: nil) ||> ** 
...
Lemma qpend_absimp_idle :
  forall P ecbls tcbls t ct st msg v x,
    ...
    can_change_aop P ->
    absimp sch (<|| rapiop (qpend, (Tint8, Tptr OS_EVENT :: Tint16 :: nil)) (Vptr x :: Vint32 v :: nil) ||> ** HECBList ecbls ** HTCBList tcbls ** HTime t ** HCurTCB ct ** P)
...
Lemma absimp_qcre_err_return :
  forall P i, 
    ...
    absimp sch ( <|| rapiop (qcre, (OS_EVENT ∗, Int16u :: nil)) (Vint32 i :: nil) ||> ** P) ( <|| rendop (Some Vnull) ||> ** P).
Proof.
  absimp_step 1%nat.
Qed.
...
Lemma absimp_qcre_succ_return' :
  forall P i qid  qls  tcbls curtid tm,
    ...
    absimp sch ( <|| rapiop (qcre, (OS_EVENT ∗, Int16u :: nil)) (Vint32 i :: nil) ||>
...
Lemma absimp_qdel_err1_return :
  forall P , 
    can_change_aop P ->
    absimp sch (<|| rapiop (qdel, (Int8u, OS_EVENT∗ :: nil)) (Vnull :: nil) ||> ** P) ( <|| rendop (Some  (V$ OS_ERR_PEVENT_NULL)) ||> ** P).
Proof.
  absimp_step 1%nat.
Qed.
...
Lemma absimp_qdel_err2_return :
  forall x a P , 
    can_change_aop P ->
    ~ (exists x0 y wls, EcbMod.get x a = Some (absmsgq x0 y, wls)) ->
    absimp sch (<|| rapiop (qdel, (Int8u, OS_EVENT∗ :: nil)) (Vptr a :: nil) ||> ** HECBList x **
...
Lemma absimp_qdel_succ_return :
  forall ecbls ecbls' x y a P tid tcbls t, 
    ...
    absimp sch (<|| rapiop (qdel, (Int8u, OS_EVENT∗ :: nil)) (Vptr a :: nil) ||> ** HECBList ecbls **
                    HTCBList tcbls ** HTime t **  HCurTCB tid **  P)
           ( <|| rendop (Some  (V$NO_ERR)) ||> ** HECBList ecbls' ** HTCBList tcbls ** HTime t **  HCurTCB tid  ** P).
Proof.
  intros.
  apply absimp'_imp_absimp; unfold absimp'; intros.
...
Lemma absimp_qdel_err3_return :
  forall x a P , 
    can_change_aop P ->
    (exists d, EcbMod.get x a = Some d /\ ~ (exists x y wls, d = (absmsgq x y, wls))) ->
    absimp sch (<|| rapiop (qdel, (Int8u, OS_EVENT∗ :: nil)) (Vptr a :: nil) ||> ** HECBList x **
...
(* ------------------- OS MUTEX Accept -------------------------*)
Lemma mutexacc_null_absimp:
  forall P,
    can_change_aop P ->
    absimp sch ( <|| rapiop (mutexacc, (Tint8, Tptr OS_EVENT :: nil)) (Vnull :: nil) ||>  ** P) (<|| rendop (Some (V$0)) ||>  ** P).
Proof.
  absimp_step 1.
Qed.
...
Lemma mutexcre_error_absimp :
  forall P i, 
    ...
    absimp sch ( <|| rapiop (mutexcre, (OS_EVENT ∗, Int8u :: nil)) (Vint32 i :: nil) ||> ** P)
           ( <|| rendop (Some Vnull) ||> ** P).
Proof.
  absimp_step 1%nat.
Qed.
...
Lemma mutexcre_succ_absimp :
  forall P i qid  qls  tcbls curtid tm,
    ...
    EcbMod.get qls qid = None  ->
    absimp sch ( <|| rapiop (mutexcre, (OS_EVENT ∗, Int8u :: nil)) (Vint32 i :: nil) ||>
                     **HECBList qls **   HTCBList tcbls ** HTime tm **  HCurTCB curtid ** P)
           ( <|| rendop (Some (Vptr qid)) ||> **                                                                                                                 
...
Lemma mutexdel_null_absimp:
  forall P , 
    can_change_aop P ->
    absimp sch (<|| rapiop (mutexdel, (Int8u, OS_EVENT∗ :: nil)) (Vnull :: nil) ||> ** P)
...
Lemma mutexdel_no_mutex_err_absimp:
  forall x a P , 
    can_change_aop P ->
    EcbMod.get x a = None ->
    absimp sch (<|| rapiop (mutexdel, (Int8u, OS_EVENT∗ :: nil)) (Vptr a :: nil) ||> ** HECBList x ** P)
...
Lemma mutexdel_type_err_absimp:
  forall ecbls a P X tcbls tm curtid, 
    ...
    absimp sch (<|| rapiop (mutexdel, (Int8u, OS_EVENT∗ :: nil)) (Vptr a :: nil) ||> ** HECBList ecbls ** HTCBList tcbls ** HTime tm **  HCurTCB curtid ** P)
...
Lemma mutexdel_succ_absimp:
  forall x a P z y x' tid t tcbls, 
    ...
    absimp sch (<|| rapiop (mutexdel, (Int8u, OS_EVENT∗ :: nil)) (Vptr a :: nil) ||> **
...
Lemma mutexpend_null_absimp:
  forall P tl vl, 
    can_change_aop P ->
    tl_vl_match tl vl = true ->
    absimp sch (<|| rapiop (mutexpend, (Int8u, OS_EVENT∗ :: tl)) (Vnull :: vl) ||> ** P)
           ( <|| rendop (Some  (V$ OS_ERR_PEVENT_NULL)) ||> ** P).
Proof.
  absimp_step 1%nat.
Qed.
...
Lemma mutexpend_no_mutex_err_absimp:
  forall x a P v, 
    ...
    can_change_aop P ->
    EcbMod.get x a = None ->
    absimp sch (<|| rapiop (mutexpend, (Int8u, OS_EVENT∗ :: Tint16 :: nil)) (Vptr a ::Vint32 v:: nil) ||> ** HECBList x ** P)
           ( <|| rendop (Some  (V$ OS_ERR_PEVENT_NO_EX)) ||> ** HECBList x ** P).
Proof.
  absimp_step 2.
Qed.
...
Lemma mutexpend_type_err_absimp:
  forall ecbls a P X tcbls tm curtid v,
    ...
    EcbMod.get ecbls a = Some X ->
    (~ exists x y wl, X = (absmutexsem x y, wl))->
    absimp sch (<|| rapiop (mutexpend, (Int8u, OS_EVENT∗ :: Tint16 :: nil)) (Vptr a :: Vint32 v :: nil) ||> ** HECBList ecbls ** HTCBList tcbls ** HTime tm **  HCurTCB curtid ** P)
           ( <|| rendop (Some  (V$OS_ERR_EVENT_TYPE)) ||> ** HECBList ecbls ** HTCBList tcbls ** HTime tm **  HCurTCB curtid ** P).
Proof.
  absimp_step 3.
Qed.
