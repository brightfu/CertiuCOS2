...
static  void  OS_InitSTKList(void);
/*$PAGE*/
/*
*********************************************************************************************************
*                                             INITIALIZATION
...
void  OSIntExit (void)
{
  ...
  //          OSIntNesting--;
  //      }
  //      if ((OSLockNesting == 0)) { /* Reschedule only if all ISRs complete ... */
...
static  void  OS_InitSTKList()
{
	...
	pstk1 = &OSSTKTbl[0];
	pstk2 = &OSSTKTbl[1];
	for (i = 0; i < (OS_MAX_TASKS + OS_N_SYS_TASKS ); i++) {  /* Init. list of free STKs                  */
	pstk1->nextblk = pstk2;
	pstk1++;
	pstk2++;
	}
	pstk1->nextblk = (OS_STK_BLK *)0;                              /* Last OS_STK_BLK                              */
	OSSTKFreeList    = &OSSTKTbl[0];
...
void  OS_Sched (void)
{
    ...
    OS_ENTER_CRITICAL();
  //  if (OSLockNesting == 0) { /* Sched. only if all ISRs done & not locked    */
...
void OS_QPtrAdd (OS_EVENT *pevent)
{
    OS_Q_PTR *p;
    p = OSQPtrFreeList;
    OSQPtrFreeList = OSQPtrFreeList->next;
    p->qeventptr = pevent;
    p->next = OSQPtrList;
    OSQPtrList = p;
}
...
void OS_QPtrRemove (OS_EVENT *pevent)
{
    OS_Q_PTR *p1,*p2;
    p2 = OSQPtrList;
    if (p2->qeventptr == pevent)
    {
        OSQPtrList = OSQPtrList->next;
        p2->next = OSQPtrFreeList;
        OSQPtrFreeList = p2;
    }
    else
    {
        while (p2 != (OS_Q_PTR *)0 && p2->qeventptr != pevent)
        {
            p1 = p2;
            p2 = p2->next;
        }
...
BOOLEAN OS_QPtrSearch (OS_EVENT *pevent)
{
    OS_Q_PTR *p;
    p = OSQPtrList;
    while (p != (OS_Q_PTR *)0 && p->qeventptr != pevent)
    {
        p = p->next;
    }
    if (p != (OS_Q_PTR *)0)
...
void OS_SemPtrAdd (OS_EVENT *pevent)
{
    OS_SEM_PTR *p;
    p = OSSemPtrFreeList;
    OSSemPtrFreeList = OSSemPtrFreeList->next;
    p->qeventptr = pevent;
    p->next = OSSemPtrList;
    OSSemPtrList = p;
}
...
void OS_SemPtrRemove (OS_EVENT *pevent)
{
    ...
    if (p2->qeventptr == pevent)
    {
        OSSemPtrList = OSSemPtrList->next;
        p2->next = OSSemPtrFreeList;
        OSSemPtrFreeList = p2;
    }
    else
    {
        while (p2 != (OS_SEM_PTR *)0 && p2->qeventptr != pevent)
        {
            p1 = p2;
            p2 = p2->next;
        }
...
BOOLEAN OS_SemPtrSearch (OS_EVENT *pevent)
{
    OS_SEM_PTR *p;
    p = OSSemPtrList;
    while (p != (OS_SEM_PTR *)0 && p->qeventptr != pevent)
    {
        p = p->next;
    }
    if (p != (OS_SEM_PTR *)0)
