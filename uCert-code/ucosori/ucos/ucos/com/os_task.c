                        ...
                        INT16U   opt)
{
    ...
    if (OSTCBPrioTbl[prio] == (OS_TCB *)0) { /* Make sure task doesn't already exist at this priority  */
        ...
        if (((opt & OS_TASK_OPT_STK_CHK) != 0x0000) ||   /* See if stack checking has been enabled     */
            ...
            #if OS_STK_GROWTH == 1
            (void)os_memset(pbos, 0, stk_size * sizeof(OS_STK));
            #else
            (void)os_memset(ptos, 0, stk_size * sizeof(OS_STK));
            #endif
        }
...
INT8U  OSTaskQuery (INT8U prio, OS_TCB *pdata)
{
    ...
    os_memcpy(pdata, ptcb, sizeof(OS_TCB));                   /* Copy TCB into user storage area          */
    OS_EXIT_CRITICAL();
    return (OS_NO_ERR);
}
#endif
